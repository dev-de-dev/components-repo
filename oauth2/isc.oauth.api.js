( function() {
  ...
  /* @ngInject */
  function iscOauthApi( $window, $http, $q, $httpParamSerializerJQLike, $http, iscOauthService ) {
    ...
    function formPost( url, data, headers ) {
      ...
      return $http.post( url, $httpParamSerializerJQLike( data ), { "headers": headers, showLoader: true } );
    }
    ...
    function doOauthCheck( stateParams ) {
      ...
      var isSameState = params.state === window.sessionStorage.getItem( 'oauthState' );
    ...
    function configureBaseUrl( stateParams , appConfig) {
    ...
    function setBaseUriFromFhir( fhirServerUrl, oauthConfig, params ) {
      ...
      return $http.get( fhirServerUrl, { showLoader: true } ).then( function( response ) {
        ...
        window.sessionStorage.setItem( "fhirMetaData", JSON.stringify(response) );
        return baseUri;
      } );
    }
